...
use naia_shared::{ChannelIndex, ExternalEntity, Protocolize};

use super::{server::Server, user::UserKey};

pub struct UserScopeMut<'s, P: Protocolize, E: ExternalEntity + Send + Sync, C: ChannelIndex> {
    server: &'s mut Server<P, E, C>,
    key: UserKey,
}

impl<'s, P: Protocolize, E: ExternalEntity + Send + Sync, C: ChannelIndex>
